...
## <--[wb]-- Path to $THIS_EXP (method 2)
# if [ $THIS_EXP = 'stiffness' ]; then
#     EXP_PATH=(${EXPCONDS_stiffness['exp_path']})
#     MOVIE_PATH=(${EXPCONDS_stiffness['movies_path']})
#     CONDITIONFILE_PATH=(${EXPCONDS_stiffness['conditionfile_path']})
# elif [ $THIS_EXP = 'mass' ]; then
#     EXP_PATH=(${EXPCONDS_mass['exp_path']})
#     MOVIE_PATH=(${EXPCONDS_mass['movies_path']})
#     CONDITIONFILE_PATH=(${EXPCONDS_mass['conditionfile_path']})
# else 
...
if [ -z $BUILDCONT ]; then
    ...
    if [ $BUILDCONT = "pull" ]; then
        ...
        # <--[wb]-- Build sandbox
        #SINGULARITY_TMPDIR=$PWD/.tmp sudo -E singularity build --sandbox $CONT/ Singularity
    fi
...
if [ $SETUPDATA = true ]; then
    ...
    # mv "$DATAPATH/data" "$DATAPATH/movies" || (echo_red "[ERROR: line ${LINENO}] $curFile" && exit 1)
    rm "movies.zip"
...
#<--[wb]-- Create movie alias
# for i in "${MOVIE_PATHS[@]}"; do
#     ln -sf $DATAPATH/movies $i
# done
...
#ln -sf "$DATAPATH/psiturk/static/data/instructions" "$EXP_PATH/psiturk/static/data/"
...
#<--[wb]-- backup existing participants.db
